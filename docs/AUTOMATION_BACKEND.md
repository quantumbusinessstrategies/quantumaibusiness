...
- `GET /api/daily-digest`: sends a protected daily owner command digest through the configured scheduler.
...
## Owned Deployment Path

Keep `quantumaibusiness.com` on GitHub Pages and deploy API routes on the same owned domain at `https://quantumaibusiness.com/api/*`.

1. Deploy the API host, preferably the Cloudflare Worker in `cloudflare/`.
2. Add these backend environment variables or secrets:
   ...
   - `CRON_SECRET=` a private random token used by scheduled jobs for the daily owner digest
...
3. Route `quantumaibusiness.com/api/*` to the API host.
...
2. Add endpoint:
   - `https://quantumaibusiness.com/api/stripe-webhook`
3. Listen for:
   - `checkout.session.completed`
4. Copy the signing secret that starts with `whsec_`.
5. Add it to the API host as `STRIPE_WEBHOOK_SECRET`.
...
- Add `OWNER_ACTION_TOKEN` to the API host.
...
The repo includes a cron-compatible route:

- Path: `/api/daily-digest`
- Schedule: `0 14 * * *`

That is 14:00 UTC daily, which is usually morning in the United States. Add `CRON_SECRET` to the API host before relying on the cron. The endpoint can also be run manually with the owner token by calling it with an `X-Owner-Token` header.
